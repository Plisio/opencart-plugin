...
<?php $invoice_is_currency_awaiting = count($allowed_currencies) > 1 and in_array($status, ['new']); ?>
<?php $invoice_is_min_sum_in_error = count($allowed_currencies) == 1 and $amount < $allowed_currencies[$currency]['min_sum_in']; ?>
<?php $invoice_is_processing = $invoice_is_min_sum_in_error != true and in_array($status, ['new', 'pending']); ?>
<?php $invoice_is_payment_waiting = in_array($status, ['new']) or (in_array($status, ['pending']) and $pending_amount > 0); ?>
<?php $invoice_is_waiting_for_confirmations = in_array($status, ['pending']) and $pending_amount <= 0; ?>
<?php $invoice_is_overpaid = in_array($status, ['mismatch']); ?>
<?php $invoice_is_finished = in_array($status, ['finish', 'completed']); ?>
<?php $invoice_is_expired = in_array($status, ['expired', 'cancelled']); ?>
<?php $invoice_is_underpaid = $invoice_is_expired and $pending_amount < $amount; ?>
<?php $invoice_is_error = in_array($status, ['error']); ?>
          ...
          <div class="col-xs-8 text-right">
            <div class="invoice__amount">
              <div class="invoice__amountSum">
                <strong class="invoice__amountSum_crypto"><?php echo number_format($amount, 8, '.', ' '); ?> <?php echo htmlspecialchars($currency); ?></strong> <br>
                <?php echo number_format($amount / $source_rate, 2); ?> <?php echo htmlspecialchars($source_currency); ?>
              </div>
            </div>
          </div>
        </div>
      ...
      <div class="invoice__content">
        ...
        <div class="text-center">
          ...
          <div class="invoice__currencyRow">
            ...
            <?php if ($allowed_curr['cid'] == $currency) { ?>
            <?php $currencyAmount = $amount; ?>
            <?php  } else if ($commission_payment == 1) { ?>
            <?php $currencyAmount = $amount / $source_rate * $allowed_curr['fiat_rate']; ?>
            ...
            <div class="invoice__currencyItem">
              ...
              <span class="invoice__input_radioBtn">
                      <span <?php if ($currency == $allowed_curr['cid']) { echo 'class="invoice__currencyContent _checked"'; } else { echo 'class="invoice__currencyContent"'; } ?> >
                        <img
        ...
        <div class="invoice__result text-center">
          ...
          <p><?php echo $text_step_error_hint_1; ?>
            <br>
            <?php  echo sprintf($text_step_error_hint_2, $allowed_currencies[$currency]['min_sum_in'] . $currency); ?>
          </p>
        </div>
        ...
        <div class="form-group">
          <div class="input-group">
            ...
            <input
                    ...
                    value="<?php echo number_format($pending_amount, 8, '.', ''); ?>"
                    readonly
        ...
        <div class="invoice__result text-center">
          ...
          <a href="<?php echo htmlspecialchars($txUrl); ?>" title="<?php echo $text_link_external; ?>" target="_blank" rel="noopener">
        ...
        <div class="invoice__result text-center">
          ...
          <p><?php echo sprintf($text_step_overpaid_hint, abs($amount + $pending_amount) . $currency) ?></p>
          <a href="<?php echo htmlspecialchars($txUrl); ?>" title="<?php echo $text_link_external; ?>" target="_blank" rel="noopener">
            <svg class="invoice__icon_btn" xmlns="http://www.w3.org/2000/svg">
              <use xlink:href="#icon_invoice_link_external"></use>
            </svg>
        ...
        <div class="invoice__result text-center">
          ...
          <p><?php echo sprintf($text_step_underpaid_hint, ($amount - $pending_amount) . $currency, $amount . $currency); ?></p>
        </div>
...
<script async defer>
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    if (!window.checkInvoiceInterval){
      window.checkInvoiceInterval = setInterval(function () {
        $.ajax({
          type: 'GET',
          url:'/index.php?route=payment/plisio/invoice',
          dataType: 'json',
          data: {
  ...
  function invoiceChooseCurrency (currency, newAmount) {
    ...
    jQuery.ajax({
      type: 'POST',
      url: '/index.php?route=payment/plisio/choosecurrency',
      dataType: 'json',
      data: {
